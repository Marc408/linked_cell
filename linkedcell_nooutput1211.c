    ...
    for (ic[0] = 0; ic[0] < nc[0]; ic[0]++)
        ...
        for (ic[1] = 0; ic[1] < nc[1]; ic[1]++)
...

            {
                ...
                while (NULL != i)
                {
                    printf("  Current i is %c at %.4f | %.4f in cell %d pointing to %c at %.4f | %.4f\n",i->p.body,i->p.x[0], i->p.x[1], index(ic,nc),i->next->p.body,i->next->p.x[0], i->next->p.x[1]);
                    //check if i is head, then do not perform anything on it
                        ...
                        for (int d = 0; d < DIM; d++)
                        {
                            ...
                            if (i->p.x[d] < 0){
                                i->p.x[d] = l[d] + i->p.x[d];
                                printf(" corrected boundaries for < 0 \n");
                            }
                            if (i->p.x[d] >= l[d]){
                                i->p.x[d] = 0 + (i->p.x[d] - l[d]);
                                printf(" corrected boundaries for > %.2f\n", l[d]);
                            }
                        }
                        ...
                        )
                        {
                            ...
                            printf("  Moved Particle %c at %.4f | %.4f from cell %d to cell %d\n", i->p.body,i->p.x[0], i->p.x[1], index(ic,nc), index(kc,nc));
                            //if (i->next == NULL)
                            //    i = *q;
                        }
                        else
...
void outputdata(real delta_t, real t_end, double timeused, int N_A, int N_B, real l[DIM])
{
    ...
    else
    {
        ...
        fprintf(fpdata, "\nLINKED-CELL,%f,%f,%d,%d,%.0f,%.0f,%.2f", delta_t, t_end, N_A, N_B, l[0], l[1], timeused);
        fclose(fpdata);
    }
}
